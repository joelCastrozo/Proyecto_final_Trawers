...
app.valoresDeTexto_up, app.valoresDeTexto_mid, app.valoresDeTexto_down = -6, -5, -4 # Declara el estado actual del texto explicativo de las escenas
...
def crear_edificio_de_fondo(x, h=140, o=60):
    ...
    for cx in range(3):
        for cy in range(12):
            fondo_de_río.agregar(Rect(x + 16.5 * cx, (h + 10) + 15 * cy, 17, 15, relleno=gradient('azulClaro', rgb(224-cy, 255, 255-cx), inicio='inferior-izquierda'), borde='negro', anchuraDeBorde=0.1, opacidad=o/1+cx*cy))
...
arbol_DE, arbol_DE.visible = Group(Line(300,220,300,285, relleno='madera',anchuraDeLinea=10), Circle(297,195,20,relleno='verdeOscuro'),
            Circle(315,218,20,relleno='verdeOscuro'),Circle(315,195,10,relleno='verdeOscuro'), Circle(289,215,10,relleno='verdeOscuro'),
            Circle(280,212,15,relleno='verdeOscuro')), False
...
def enTeclaPresionada(tecla):
    ...
    if tecla == 'derecha':
        ...
        if app.estado == 'En escena 0.5':
            ...
            quitarVallas()
            sol.relleno = gradient('caqui', rgb(255, 120, 0), 'oro')
            ...
            sol.relleno = gradient('caqui', rgb(255, 165, 0), 'oro')
            ...
            sol.relleno = gradient('caqui', rgb(255, 205, 0), 'oro')
        elif app.estado == 'En escena de campaña': 
            app.fondo = 'azulCieloProfundo'
            escenaDeSalvaciónDeBosque.vaciar(), escenaDeReciclaje.agregar(sol, nube_1, nube_2, fondo_montañoso, suelo, arbóles, camino)
            ...
            nube_1.relleno, nube_2.relleno = 'nieve', 'nieve'
            sol.relleno = gradient('caqui', rgb(255, 255, 0), 'oro')
            app.cartel_estado = 'Abajo'
...
def enRatónArrastrado(ratónX, ratónY):
    ...
    if app.estado == 'En escena de minijuego I':
        ...
        if app.objeto.contiene(ratónX, ratónY):
            ...
            palabrasDeAnimo = ['¡Muy bien!', '¡Sigue así!', '¡En hora buena!', '¡Sabés lo que haces!', '¡Recicle bucle!', '¿Estás haciendo trampa?', '¡¡Perfecto!!', 'GG', 'Vamos']
            if app.correcto == False:
                texto.valor = '¡Mueva el objeto al grupo correcto!'
                texto.relleno = 'amarillo'
            if app.correcto == True:
                i = randrange(len(palabrasDeAnimo))
